(function () {
    ...
    function main() {
        jQuery(document).ready(function ($) {
            ...
            /******* Load HTML *******/
            var url = "https://bundesliga-widgets.de/Widgets/Simple";
            ...
            $.get(url, function (data) {
                $('#fblwidget_matches').html(data);

                if (typeof fblWidgetSettings != 'undefined') {
                    $('#fblwidget_matches').find('.blwTeamName').each(function (index, team) {
                        // Highlight Teams:
                    ...
                    $('#fblwidget_matches').find('.blwTeamNameMatch').each(function (index, team) {
                        // Match-Links:
                        if (typeof fblWidgetSettings.matchLinks != 'undefined') {
                            fblWidgetSettings.matchLinks.forEach(function (item) {
                                if ($(team).text().indexOf(item.teamname) >= 0) {
                                    $(team).parent().parent().parent().find(".blwResult").wrap('<a href= "' + item.link + '" target= "_blank" class="blwTableCellCondensed" />');
                                }
                            });
                        }
                    });
